...
/// PVC Membrane Calculator - Calculate PVC roofing materials
class PvcMembraneScreen extends ConsumerStatefulWidget {
  const PvcMembraneScreen({super.key});
  @override
  ConsumerState<PvcMembraneScreen> createState() => _PvcMembraneScreenState();
}

class _PvcMembraneScreenState extends ConsumerState<PvcMembraneScreen> {
  ...
  void _calculate() {
    ...
    // Rolls needed (assume 100 ft roll length)
    final rollCoverage = rollWidthFt * 100;
    final fieldRolls = (roofArea * 1.1 / rollCoverage).ceil();

    // Flashing rolls (18" typical width for details)
    ...
    final seamCount = (50 / rollWidthFt).ceil();
    final seamLength = roofLength * seamCount;

    // Weld rod for hot-air welding
    final weldRodFeet = seamLength / 2;

    // Termination bar (linear feet for parapet)
    final terminationBar = (parapetLength * 1.1).ceil();
    ...
    setState(() {
      ...
      _terminationBar = terminationBar;
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('PVC Membrane', style: TextStyle(color: colors.textPrimary, fontWeight: FontWeight.w600)),
        actions: [
          IconButton(
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Text(
            'Calculate polyvinyl chloride roofing materials',
  ...
  Widget _buildThicknessSelector(ZaftoColors colors) {
    final thicknesses = ['50 mil', '60 mil', '80 mil'];
    return Row(
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 8),
          _buildResultRow(colors, 'Termination Bar', '$_terminationBar lin ft'),
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'PVC is chemically resistant and self-extinguishing. Weld at 800-1000°F.',
