// ZAFTO — DeWalt BLE Laser Meter Adapter (BETA)
// Created: Sprint FIELD4 (Session 131)
//
// Handles DeWalt BLE laser meters (DW099S, DW0165, etc.).
// DeWalt uses a custom BLE profile. Ships as BETA.
//
// DeWalt BLE protocol (reverse-engineered):
// - Custom service UUID
// - Measurement data: varies by model, typically 4-byte float in meters
...
// =============================================================================
// DEWALT BLE CONSTANTS
// =============================================================================

/// DeWalt custom measurement service UUID.
const String _kDewaltMeasurementServiceUuid =
    '6e40fff0-b5a3-f393-e0a9-e50e24dcca9e';

/// DeWalt measurement characteristic UUID.
const String _kDewaltMeasurementCharUuid =
...
class DewaltAdapter implements LaserMeterAdapter {
  ...
  @override
  LaserMeterBrand get brand => LaserMeterBrand.dewalt;

  @override
  List<String> get serviceUuids => [_kDewaltMeasurementServiceUuid];

  @override
  bool canHandle(String deviceName, List<int>? manufacturerData) {
    final nameLower = deviceName.toLowerCase();
    if (nameLower.contains('dewalt') || nameLower.contains('dw0')) return true;

    // Stanley Black & Decker manufacturer ID (0x02C6)
    if (manufacturerData != null && manufacturerData.length >= 2) {
      final companyId = manufacturerData[0] | (manufacturerData[1] << 8);
      if (companyId == 0x02C6) return true;
    }
    ...
    try {
      ...
      for (final service in services) {
        if (service.uuid.toString().toLowerCase() ==
            _kDewaltMeasurementServiceUuid) {
          measurementService = service;
          break;
        }
      }

      // Fallback: find any service with notify characteristics
      ...
      for (final char in measurementService.characteristics) {
        ...
        if (cid == _kDewaltMeasurementCharUuid ||
  ...
  Future<LaserDeviceInfo> getDeviceInfo() async {
    if (_device == null) {
      return LaserDeviceInfo(
          deviceId: '', name: 'Unknown DeWalt', brand: brand);
    }
  ...
  /// Parse DeWalt measurement packet.
  /// DeWalt typically uses IEEE 754 float (little-endian) in meters or feet.
  void _parseMeasurement(List<int> value, String deviceId) {
    ...
    try {
      ...
      // DeWalt DW099S reports in feet — detect by checking if value
      // is reasonable as feet (< 165 = 50m range)
      MeasurementSourceUnit unit = MeasurementSourceUnit.meters;
      double inches;

      if (meters.isNaN || meters.isInfinite || meters < 0) return;

      if (meters > 100 && meters < 600) {
        // Likely feet (Bosch max range ~100m, but 300+ feet is common)
        unit = MeasurementSourceUnit.feet;
        inches = meters * 12.0; // "meters" variable actually holds feet
      } else if (meters > 600) {
        return; // Unreasonable
      } else {
