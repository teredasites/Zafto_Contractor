...
| Field | Value |
|-------|-------|
| **Sprint** | **S137: ENTERPRISE INFRASTRUCTURE + INFRA-1→INFRA-5 (~8h).** Supabase Pro + Vercel Pro setup. 4-environment pipeline (local/preview/staging/production). Database performance indexes. 3-layer architecture enforcement (UI never breaks backend). PITR flagged as LAUNCH BLOCKER ($100/mo, enable before go-live). Full research in `memory/enterprise-infrastructure-research-s137.md`. |
| **Sub-step** | **INFRA-1→INFRA-5 (~8h) → TEST-INFRA (TI-3/TI-5/TI-7 remaining) → COLLAB-ARCH (CA-1→CA-5) → DEPTH2-39.** Execution order: **INFRA-1→5(NEXT)** → TEST-INFRA → COLLAB-ARCH → DEPTH2-39 → DEPTH40-nonAI → DEPTH41-43 → **INTEG2-4 + INTEG6-8 (pre-RE ecosystem wiring, ~248h)** → RE1-20 → INTEG1 → FLIP1-4 → **INTEG5 (three-sided marketplace, after RE+FLIP)** → SEC2-5 → LAUNCH2-6 → LAUNCH8 → G → JUR → E → FLIP5+DEPTH40-AI+DEPTH44 → SEC9-10 → ZERO1-9 → LAUNCH7 → **[PITR ON]** → SHIP. |
