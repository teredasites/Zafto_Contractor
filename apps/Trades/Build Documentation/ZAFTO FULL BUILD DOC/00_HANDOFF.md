...
| Field | Value |
|-------|-------|
| **Sprint** | **S152+: DEPTH8-25 ALL COMPLETE + SEC SOURCE PROTECTION + PRICING REWRITE.** DEPTH8: 6 onboarding fixes. DEPTH9: 4 trade customization fixes. DEPTH12-14: 1 settings fix + offline/sec gap docs. DEPTH15-22: 0 corrections (all clean). DEPTH23-24: Codebase sweep clean. DEPTH25: 120+ commercial symbols, 16 building templates, flat roof/fire protection/ADA. SEC: 7-layer source protection on all 4 portals + right-click restored. Rule #24 ZERO HARDCODED PRICING: estimate-generator.ts rewritten with DB-backed pricing cascade (company→MSA→national). trade-formulas.ts costPerSqFt removed. Commits: b41a221→f963f45 (8 commits). |
| **Sub-step** | **DEPTH25(DONE)** → **DEPTH26(NEXT)** → DEPTH27-39 → DEPTH40-nonAI → DEPTH41-43 → DATA-ARCH1-4 → INTEG6→2→3→4→7→8 → S135-ENTITY → RE1-20 → RE21-33 → ADJ1-12 → INTEG1 → FLIP1-6 → **FLIP-PORTAL1-11(S150)** → INTEG5 → SEC2-5 → LAUNCH2-6 → LAUNCH8 → G → JUR → E → FLIP5+DEPTH40-AI+DEPTH44 → SEC9-10 → ZERO1-9 → LAUNCH7 → LAUNCH-FLAVORS → APP-DEPTH → **[PITR ON]** → SHIP. |
...
| **Migration Files** | 129 total. Latest: depth corrections, SEC audit fixes, walkthrough corrections. |
