...
| Field | Value |
|-------|-------|
| **Sprint** | **S130: OWNER DIRECTIVES.** 27 items from owner's handwritten notes integrated into sprint specs. 8 new DEPTH sprints (DEPTH37-44, ~140h). 12 additions to existing sprints (FIELD1, G9, SEC10, DEPTH4/10/12/13/19/22/28/29/34, BA2, SK13-EXP). Key new features: tablet/mobile responsive overhaul, time clock adjustment by managers, signature system (DocuSign replacement at $0), universal marketplace aggregator with AI, backup fortress (Supabase+R2+B2 triple redundancy), storage tiering (5GB free + paid), sketch engine file import/export, AI access controls + AI ticket system. Legal labeling standard from owner's legal doc baked into SEC10. ~102 sprints (~1,716h). |
| **Sub-step** | **SEC1+SEC6-8 next (critical security fixes — site is live), then LAUNCH1 (monitoring), then FIELD1 (team messaging + phone BYOC).** DEPTH37-44 added to execution order (after DEPTH36, before RE1-RE20). DEPTH40+DEPTH44 are Phase E dependent. Full execution order in PHASE AUDIT SUMMARY section of 07_SPRINT_SPECS.md. |
