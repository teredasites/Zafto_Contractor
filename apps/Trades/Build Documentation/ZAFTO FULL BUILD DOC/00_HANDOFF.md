...
| Field | Value |
|-------|-------|
| **Sprint** | **S127: PP + MOLD + REALTOR + INTEG + FLIP-IT.** DEPTH34 (~52h PP), DEPTH35 (~28h mold), DEPTH36 (~8h dump finder), REALTOR1-3 (~48h realtor portal), INTEG1 (~12h national portal API), **FLIP1-4 (~52h Flip-It analysis engine — deal aggregation from 7 sources, appraisal-grade ARV, full financial model, profit degradation timeline, hard money lender directory ~100+ lenders, investor connection board).** 78 sprints (~1,078h). |
| **Sub-step** | **SEC1 next (critical security fixes — site is live), then LAUNCH1 (monitoring), then FIELD1 (team messaging).** DEPTH34-36, REALTOR1-3, INTEG1, FLIP1-4 added to execution order. Full execution order in PHASE AUDIT SUMMARY section of 07_SPRINT_SPECS.md. |
