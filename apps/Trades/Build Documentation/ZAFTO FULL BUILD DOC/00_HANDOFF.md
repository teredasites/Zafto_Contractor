...
| Field | Value |
|-------|-------|
| **Sprint** | **S131: DEPTH1 COMPLETE. NICHE1+NICHE2 ALL COMPLETE. REST1+REST2 COMPLETE. SEC1+SEC6-8 + FIELD1-5 ALL COMPLETE.** DEPTH1: Core business depth audit (4 parallel Opus agents) + 3 correction batches — Flutter customer ID linking fix, CRM mapper 6 missing fields, team portal customer contact card, client portal CO approve/reject buttons, Tasks tab wired to schedule_tasks, Materials tab wired to job_materials, invoice ghost inputs persisted (migration 000113: po_number/retainage/late_fee/discount/payment_terms), customer edit form + delete + create-job/invoice buttons. MOV1-MOV8 sprint specs added (~200h, moving company module with LiDAR volume estimation). |
| **Sub-step** | **DEPTH2 next** (field tools depth audit ~16h). Execution order: DEPTH2-39 → DEPTH40-nonAI → DEPTH41-43 → RE1-20 → INTEG1 → FLIP1-4 → SEC2-5 → LAUNCH2-6 → LAUNCH8 → G → JUR → E → FLIP5+DEPTH40-AI+DEPTH44 → SEC9-10 → ZERO1-9 → LAUNCH7 → SHIP. |
