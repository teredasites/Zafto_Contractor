...
| Field | Value |
|-------|-------|
| **Sprint** | **S127: PP + MOLD + REALTOR + INTEG + FLIP-IT.** DEPTH34 (~52h PP), DEPTH35 (~28h mold), DEPTH36 (~8h dump), REALTOR1-3 (~48h realtor portal), INTEG1 (~12h national API), **FLIP1-5 (~64h Flip-It engine — deal aggregation, ARV, financial model, profit degradation, hard money lenders, deal cross-post, AI photo analysis + premium deep assessment).** FIRST PAID ADD-ON. 79 sprints (~1,090h). |
| **Sub-step** | **SEC1 next (critical security fixes — site is live), then LAUNCH1 (monitoring), then FIELD1 (team messaging).** DEPTH34-36, REALTOR1-3, INTEG1, FLIP1-5 added to execution order. Full execution order in PHASE AUDIT SUMMARY section of 07_SPRINT_SPECS.md. |
