...
**Phase INS Inspector Deep Buildout (S121-S122):** 5 inspector screens (home, inspect, tools, history, more) + 4 new screens: code_reference_screen (61 NEC/IBC/IRC/OSHA/NFPA sections, pickMode for citation linking), compliance_calendar_screen (overdue/today/week/upcoming groups), permit_inspection_tracker_screen (8 permit types, stage mapping, CO eligibility), reinspection_diff_screen (side-by-side comparison). Data: code_reference_data.dart (61 static code sections). Services: inspection_service.dart (InspectionService + Riverpod providers), inspection_gps_service.dart (geolocator 11.x). Home screen wired to activeClockEntryProvider + timeClockStatsProvider. Tools screen: 13 wired tools + COMPLIANCE section (3 tools). Web: use-inspection-templates.ts hook, templates page, inspection [id] detail. Ops: inspector-metrics page. Team: inspections page.
