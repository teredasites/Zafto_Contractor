...
24. **ZERO HARDCODED PRICING — DATABASE OR USER DATA ONLY** — NEVER hardcode pricing, costs, rates, or financial figures anywhere in the codebase. ALL pricing MUST come from the database (`estimate_pricing` table with BLS-backed national averages, `msa_regions` for regional cost adjustment via `fn_zip_to_msa`, company-specific overrides in `estimate_pricing` where `company_id IS NOT NULL`), or from the user's custom price book data. The pricing cascade is: company override → regional (MSA) → national average. For items not yet in the database, include them with $0 and mark as 'unpriced' so the user MUST set their own price — NEVER invent approximate numbers. This applies to ALL modules: sketch engine estimates, trade calculators, material calculators, cost projections, and any future feature that outputs dollar amounts. "Ballpark" pricing is forbidden. Zafto is built on accuracy. NO EXCEPTIONS.
25. **SOURCE PROTECTION ON ALL PORTALS** — Every Next.js portal (web-portal, team-portal, client-portal, ops-portal, and any future portal) MUST include the `SourceProtection` component in its root `layout.tsx`. The component (`src/components/source-protection.tsx`) provides 6 layers of anti-snooping defense: devtools keyboard shortcut blocking (F12, Ctrl+Shift+I/J/C, Ctrl+U, Ctrl+S), debugger-based devtools detection (blanks page with "Access Denied"), console method poisoning (all methods replaced with noop), drag event blocking, text selection blocking (except form fields), and copy blocking (except form fields). Right-click is intentionally NOT blocked — it annoys users and the devtools detection catches anyone who selects "Inspect" from the context menu anyway. CSS `user-select: none` is injected on body with `user-select: text` on inputs/textareas. All protections are production-only (`NODE_ENV !== 'production'` returns early). Every portal's `next.config` MUST have `productionBrowserSourceMaps: false` and `poweredByHeader: false`. Security headers MUST include empty `SourceMap` and `X-SourceMap` to block source map requests. When creating ANY new portal, copy `source-protection.tsx` from web-portal, wire `<SourceProtection />` inside the ThemeProvider in root layout, and add the config flags. Future: custom context menus with entity-specific actions (spec as late-stage UX polish sprint after all core features are built). NO EXCEPTIONS.
