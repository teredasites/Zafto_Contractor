...
24. **SOURCE PROTECTION ON ALL PORTALS** — Every Next.js portal (web-portal, team-portal, client-portal, ops-portal, and any future portal) MUST include the `SourceProtection` component in its root `layout.tsx`. The component (`src/components/source-protection.tsx`) provides 7 layers of anti-snooping defense: right-click blocking, devtools keyboard shortcut blocking (F12, Ctrl+Shift+I/J/C, Ctrl+U, Ctrl+S), debugger-based devtools detection (blanks page with "Access Denied"), console method poisoning (all methods replaced with noop), drag event blocking, text selection blocking (except form fields), and copy blocking (except form fields). CSS `user-select: none` is injected on body with `user-select: text` on inputs/textareas. All protections are production-only (`NODE_ENV !== 'production'` returns early). Every portal's `next.config` MUST have `productionBrowserSourceMaps: false` and `poweredByHeader: false`. Security headers MUST include empty `SourceMap` and `X-SourceMap` to block source map requests. When creating ANY new portal, copy `source-protection.tsx` from web-portal, wire `<SourceProtection />` inside the ThemeProvider in root layout, and add the config flags. NO EXCEPTIONS.
